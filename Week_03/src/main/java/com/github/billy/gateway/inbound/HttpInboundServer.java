...
public class HttpInboundServer {
    ...
    public HttpInboundServer(int port,ProxyServerOption option) {
    ...
    public void run() throws Exception {
        ...
        try {
            ...
            b.group(bossGroup, workerGroup).channel(NioServerSocketChannel.class)
                    .handler(new LoggingHandler(LogLevel.INFO)).childHandler(new HttpInboundInitializer(this.proxyServerOption));

            Channel ch = b.bind(inboundServerPort).sync().channel();
            //logger.info("开启netty http服务器，监听地址和端口为 http://127.0.0.1:" + port + '/');
            System.out.println("开启netty http服务器，监听地址和端口为 http://127.0.0.1:" + inboundServerPort + '/');
            ch.closeFuture().sync();
        } finally {
